...
- **AND** the public wishlist page renders the selected motif after revalidation

#### Scenario: Motif controls are absent for ungated event types

- **WHEN** the settings form renders for a wishlist whose event type is not `baby_shower` or `birthday`
- **THEN** no motif controls are shown and the wishlist's motif columns stay null
