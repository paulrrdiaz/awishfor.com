...
- [x] 2.8 Verify each animal shape (including `elephant`) at native size and at 0.18x (smallest current placement scale) — confirm the silhouette and face remain readable as the intended animal, not just as a colored blob
...
- [x] 6.2 Update `src/components/shared/motif/motif-shape.stories.tsx` to drop `horn` and confirm all 16 remaining shapes still render through Storybook controls at multiple scales/surfaces
...
- [x] 9.2 Re-author `dino` in the outline/shading style, referencing the pasted chibi-triceratops and hatching-dino images
- [x] 9.3 Re-author `bunny` in the outline/shading style, referencing the pasted floral-crown bunny image
- [x] 9.4 Re-author `duck` in the outline/shading style, referencing the pasted rubber-duck images
- [x] 9.5 Re-author `moon` in the outline/shading style, referencing the pasted kawaii moon/cloud/stars card
- [x] 9.6 Re-author `elephant` in the outline/shading style (no direct reference pasted this round — reuse proportions/mood from the bunny/dino references)
- [x] 9.7 Re-author `unicorn` in the outline/shading style (no direct reference pasted this round — reuse proportions/mood from the bunny/dino references)
- [x] 9.8 Show the 6 re-authored shapes to the user (Storybook or dev server) and get style sign-off before touching the remaining 10 shapes
- [x] 9.9 Once style is confirmed, re-author the remaining 10 shapes (`bear`, `fox`, `cloud`, `flower`, `rainbow`, `tree`, `boat`, `balloon`, `star`, `leaf`) to match
- [x] 9.10 Re-run `pnpm check` / `pnpm test` / `pnpm typecheck` after the full 16-shape re-author
